...
type Song = { id:number; title:string; artist:string; album:string; quality:string; fileName:string; path:string; fileId:string; format:string; duration:string; stream:string; cover?:string };
...
export default function Page(){
  ...
  const clearLoadedTrack=()=>{
    ...
    setPosition(0);setDuration(0);setStreamError('');
  ...
  const visibleSongs=useMemo(()=>{
    if(q)return allSongs.filter(song=>(`${song.title} ${song.artist} ${song.album} ${song.fileName}`).toLowerCase().includes(q));
