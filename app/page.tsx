...
const songs:Song[]=[
  ...
  {id:2,title:'M4A Test Track',artist:'Test Track',album:'My Music',quality:'ALAC M4A • Original file',cover:'linear-gradient(135deg,#059669,#164e63)',duration:'2:30',stream:'/api/track?format=m4a'}
];
...
export default function Page(){
  ...
  const alacStartRef=useRef(0);
  const alacSongIdRef=useRef<number|null>(null);

  const isAlac=current.stream.includes('format=m4a');

  const stopAlac=()=>{
    try{alacSourceRef.current?.stop();}catch{}
    alacSourceRef.current?.disconnect();
  ...
  const playAlac=async(offset:number)=>{
    try{
      const ctx=ensureContext();
      await ctx.resume();
      setStreamError('');

      if(!alacBufferRef.current || alacSongIdRef.current!==current.id){
        setLoading(true);
        const response=await fetch(current.stream,{cache:'no-store'});
        if(!response.ok) throw new Error(`MEGA stream returned ${response.status}`);
        const bytes=new Uint8Array(await response.arrayBuffer());
        const decoded=await decodeMp4(bytes);
        const channelCount=decoded.channelData.length;
        if(!channelCount) throw new Error('No audio channels found in the M4A file');
        const buffer=ctx.createBuffer(channelCount,decoded.channelData[0].length,decoded.sampleRate);
        decoded.channelData.forEach((channel,index)=>buffer.copyToChannel(channel,index));
        alacBufferRef.current=buffer;
        alacSongIdRef.current=current.id;
        setDuration(buffer.duration);
        setLoading(false);
      }

      const buffer=alacBufferRef.current;
      if(!buffer) throw new Error('ALAC decoder did not return audio');
      stopAlac();
      const source=ctx.createBufferSource();
      source.buffer=buffer;
      source.connect(ctx.destination);
      alacSourceRef.current=source;
      alacStartRef.current=ctx.currentTime-offset;
      source.onended=()=>{
        if(alacSourceRef.current===source){
          alacSourceRef.current=null;
          const pos=Math.min(buffer.duration,ctx.currentTime-alacStartRef.current);
          if(pos>=buffer.duration-0.05){setPosition(buffer.duration);setPlaying(false);} 
        }
      };
      source.start(0,Math.max(0,Math.min(offset,Math.max(0,buffer.duration-0.001))));
      setPosition(Math.max(0,offset));
      setPlaying(true);
    }catch(error){
      setLoading(false);
      setPlaying(false);
      setStreamError(error instanceof Error?`ALAC playback failed: ${error.message}`:'ALAC playback failed.');
    }
  };
  ...
  useEffect(()=>{
    ...
    const id=window.setInterval(()=>{
      ...
      const buffer=alacBufferRef.current;
      if(playing&&ctx&&buffer&&alacSourceRef.current){
        const next=Math.max(0,Math.min(buffer.duration,ctx.currentTime-alacStartRef.current));
        setPosition(next);
      }
    },250);
  ...
  useEffect(()=>()=>{stopAlac();void audioContextRef.current?.close();},[]);
  ...
  const togglePlay=()=>{
    if(isAlac){
      if(playing){
        ...
        void playAlac(position);
      }
      return;
    }
    const a=audioRef.current;
    if(!a)return;
    if(a.paused)a.play().then(()=>setPlaying(true)).catch(()=>setStreamError('Playback failed.'));
    else a.pause();
  };

  const filtered=useMemo(()=>songs.filter(s=>(s.title+s.artist+s.album).toLowerCase().includes(query.toLowerCase())),[query]);

  return <div className="min-h-screen bg-zinc-950 pb-28">
    <aside className="fixed left-0 top-0 hidden h-screen w-64 border-r border-white/10 bg-zinc-950/95 p-6 md:block"><div className="mb-10 flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-white text-black"><Music2 size={21}/></div><div><div className="font-bold">MY MUSIC</div><div className="text-xs text-zinc-500">Private library</div></div></div><nav className="space-y-2">{[[Home,'Home'],[Search,'Search'],[Library,'Your Library'],[Heart,'Favorites']].map(([I,n]:any)=><button key={n} onClick={()=>setActive(n)} className={'flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm '+(active===n?'bg-white text-black':'text-zinc-400 hover:bg-white/5 hover:text-white')}><I size={18}/>{n}</button>)}</nav><div className="mt-10 border-t border-white/10 pt-6"><div className="mb-3 flex items-center justify-between text-xs font-semibold text-zinc-500"><span>PLAYLISTS</span><Plus size={15}/></div><div className="space-y-1 text-sm text-zinc-400"><button className="w-full rounded-lg px-3 py-2 text-left hover:bg-white/5">Hi-Res Favorites</button><button className="w-full rounded-lg px-3 py-2 text-left hover:bg-white/5">Night Drive</button></div></div></aside>
